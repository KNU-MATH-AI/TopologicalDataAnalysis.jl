...
function deg(σ)
    return T.epsilon[findfirst(T.sigma .== σ)]
end
deg(test_σ)
...
function maxindex(chain)
    return (T.sigma .∈ Ref(chain)) |> findall |> maximum
end
maxindex(∂(test_σ))
...
function REMOVEPIVOTROWS(σ)
    k = dim(σ); d = ∂(σ)
    d = d[d .∈ Ref(T[T.marked,:sigma])] # Remove unmarked terms in $d$
    while !(d |> isempty)
        i = maxindex(d)
        if T[i,:d] |> isempty break end
        d = symdiff(d, T[i,:d])
        # print("d in empty")
    end
    return d

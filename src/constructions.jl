...
# We rename "weights" as "filtration_weights".
...
"""
    vietorisrips(X, ɛ[, metric, filtration_weights])
...
- `filtration_weights::Bool`: this flag indicates if the filteration weight should be calculated. Default value is `true`.
...
"""
function vietorisrips(X::AbstractMatrix{T}, ɛ::Real, metric::Metric=Euclidean(), filtration_weights=true;
    ...
    vietorisrips(D, ɛ, filtration_weights; expansion=expansion, maxoutdim=maxoutdim)
end

"""
    vietorisrips(D, ɛ[, filtration_weights])

Construction of a Vietoris–Rips complex from the distance matrix `D`.
"""
function vietorisrips(D::AbstractMatrix{T}, ɛ::Real, filtration_weights::Bool;
                      expansion = :incremental,
    ...
    # calculate filtration_weights of nerve
    w = nothing
    if filtration_weights
        @debug "Calculate filtration_weights of 1-skeleton"
        w = Dict{Int, Vector{T}}()
        w[0] = zeros(n)
        if size(cplx, 1) > 0
...
"""
    witness(X, l, ɛ[, filtration_weights])
...
"""
function witness(X::AbstractMatrix{T}, l::Int, ɛ::Real, filtration_weights = true;
    ...
    cplx, w = witness(D, ɛ, filtration_weights, expansion=expansion, ν=ν, maxoutdim=maxoutdim, firstpoint=firstpoint)

    return cplx, w
end

"""
    witness(D, ɛ[, filtration_weights])

Construction of a witness complex from the distance matrix `D` with a maximum scale value `ɛ`.
"""
function witness(D::AbstractMatrix, ɛ::Real, filtration_weights = true;
                 expansion = :incremental,
                 ν::Int = 2,
...
"""
    čech(X, ɛ[, filtration_weights])
    cech(X, ɛ[, filtration_weights])
...
"""
function čech(X::AbstractMatrix{T}, ɛ::Real, filtration_weights = true;
              maxoutdim = size(X,2)-1) where T <: Real
    ...
    for h in 1:maxoutdim
        idx = collect(h+1:-1:1)
        filtration_weights && setindex!(w, zeros(T, 0), h)

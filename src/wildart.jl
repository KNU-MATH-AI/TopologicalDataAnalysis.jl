# Hardcopy from https://github.com/wildart/SmithNormalForm.jl
...
function divisable(y::R, x::R ) where {R}
  ...
  function snf(M::AbstractMatrix{R}; inverse=true) where {R}
      rows, cols = size(M)
      if min(rows, cols) > 100
          M = convert.(BigInt,M)
          @info "Converted to BigInt"
      end
      ...
      for j in 1:cols
          ...
          t += 1
          if t % 10 == 1
              println(Dates.format(now(), "HH:MM") * " // $t : maximum = $(maximum(D))")
        #   elseif t%10 == 0
        #       print("$t >")
          end
